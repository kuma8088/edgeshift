...
export async function getArchiveList(
  ...
  try {
    ...
    // Query only sent AND explicitly published campaigns (exclude soft-deleted)
    const query = `
      SELECT * FROM campaigns
      WHERE status = 'sent' AND is_published = 1 AND is_deleted = 0
...
export async function getArchiveArticle(
  ...
  try {
    const campaign = await env.DB.prepare(`
      SELECT * FROM campaigns
      WHERE slug = ? AND status = 'sent' AND is_published = 1 AND is_deleted = 0
