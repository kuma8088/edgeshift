...
| メールアドレス | `test+yyyymmdd@edgeshift.tech` 形式（クリーンアップ対象） |
| タイミング | 08:00 JST に登録開始 |
| タイムゾーン | システムは JST で処理（DB保存も JST） |

### 本番テスト安全ルール

| ルール | 説明 |
|--------|------|
| テスト専用リスト必須 | キャンペーン配信は必ず「テスト購読者リスト」を選択。**「全購読者」は絶対に選択しない** |
| テストメールアドレス | `test+*@edgeshift.tech` 形式のみ使用。個人メールアドレスは使用禁止 |
| 命名規則 | 全テストリソースは "Test" または "テスト" で始める |
...
3. 08:00 JST に `/newsletter/signup/test-sequence` (landing ページ) から `test+yyyymmdd@edgeshift.tech` で登録
...
- [ ] Cron トリガー（15分間隔）によるスケジュール配信が正常に動作する
...
4. 新規キャンペーンを作成・配信
5. unsubscribed ユーザーに配信されないことを確認（delivery_logs に新規レコードが作成されない。抑止ログは記録しない設計）
...
**実装ファイル:** `tests/e2e/specs/02-batch-tb-automated.spec.ts`

**TB-2-1: シーケンスメール作成・配信**
- ✅ 管理画面でシーケンス作成（5ステップ、delay 設定）
- ✅ シーケンス有効化
- ✅ 購読登録（実メールアドレス使用）
- ✅ D1 で delivery_logs 検証（配信試行確認）
- ❌ 実メール受信確認（手動）
- ❌ リンク A-F クリック & Click Tracking 検証（手動）

**TB-3-1: キャンペーン作成・配信**
- ✅ 管理画面でキャンペーン作成
- ✅ RichTextEditor でリンク挿入
- ✅ 即時配信実行
- ✅ D1 で delivery_logs 検証
- ❌ 実メール受信確認（手動）
- ❌ リンク A-D クリック & Click Tracking 検証（手動）

**TB-3-2: コンタクトリスト**
- ✅ リスト作成・メンバー追加
- ✅ リスト指定配信
- ✅ D1 で配信対象検証（リストメンバーのみ配信）
- ❌ 実メール受信確認（手動）

### 手動テストのみ

以下は E2E テストでカバーされず、ユーザーが手動で実施する必要があります：

**TB-2-2: 埋め込みフォームテスト**
- theme/size パラメータの全組み合わせ確認
- 埋め込みコード生成 UI の動作確認

**TB-2-3: Unsubscribe フロー**
- Unsubscribe リンククリック
- ステータス更新確認
- 配信停止確認（翌日のシーケンスメールが届かない）
...
# テスト実行
npm run test:e2e -- tests/e2e/specs/02-batch-tb-automated.spec.ts
```

**注意事項:**
- E2E テストは管理画面操作と D1 検証のみ
- 実際のメール受信・リンククリックは手動で確認
- テスト実行後、メールボックスを確認してリンクをクリック
- 管理画面で Click Tracking が正しく記録されているか確認
...
| 2025-12-26 | v1.1 | E2E 自動化範囲セクションを追加（自動化済み項目と手動テスト項目を明示） |
| 2025-12-26 | v1.2 | レビュー指摘対応: 本番テスト安全ルール追加、テストメールアドレス統一、登録導線明記、用語修正 |
